...
public final class PhonewordUtils {
    ...
    private static final String SETTINGS_SAVED_PHONEWORDS = "congnt.phoneword.saved";
    private static final String SETTINGS_PHONEWORD = "congnt.phoneword";
    ...
    public static String formatStringAsPhoneNumber(@NotNull String phoneNumber) {
        ...
        switch (phoneNumber.length()) {
            case 7:
                output = String
                    .format("%s-%s", phoneNumber.substring(0, 3), phoneNumber.substring(3, 7));
                break;
            case 10:
                output = String
                    .format("(%s) %s-%s", phoneNumber.substring(0, 3), phoneNumber.substring(3, 6),
                        phoneNumber.substring(6, 10));
                break;
            case 11:
                output = String.format("%s (%s) %s-%s", phoneNumber.substring(0, 1),
                    phoneNumber.substring(1, 4), phoneNumber.substring(4, 7),
                    phoneNumber.substring(7, 11));
                break;
            case 12:
                output = String.format("+%s (%s) %s-%s", phoneNumber.substring(0, 2),
                    phoneNumber.substring(2, 5), phoneNumber.substring(5, 8),
    ...
    public static void savePhoneword(@NotNull Context context, @NotNull String number) {
        ...
        Set<String>
            phonewords = settings.getStringSet(SETTINGS_SAVED_PHONEWORDS, new HashSet<String>());
